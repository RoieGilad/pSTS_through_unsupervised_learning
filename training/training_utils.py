...
def run_one_batch(model, batch, loss_function):
    output_videos, output_audios = [], []
    for video, audio, label in batch:
        encode_video, encode_audio = model(video, audio)
        output_videos.append(encode_video)
        output_audios.append(encode_audio)

    output_videos = torch.stack(output_videos)
    output_audios = torch.stack(output_audios)
    return loss_function(output_videos, output_audios)
